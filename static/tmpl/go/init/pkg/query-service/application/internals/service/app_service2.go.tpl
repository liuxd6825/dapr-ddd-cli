...
type {{.Name}}QueryAppService struct {
    ...
    {{- end }}
    {{- end }}
    findByIdExecutor  *executor.FindByIdExecutor   // 按Id查询器
    findByIdsExecutor *executor.FindByIdsExecutor  // 按多个Id查询器
    findByAggregateIdExecutor *executor.FindByAggregateIdExecutor // 按聚合根Id查询器
    findAllExecutor *executor.FindAllExecutor      // 所有数据查询器
    findPagingExecutor *executor.FindPagingExecutor // 分页查询器
}
...
func (a *{{.Name}}QueryAppService) DeleteMany(ctx context.Context, tenantId string, vList []*view.{{.Name}}View) error {
	return session.StartSession(ctx, func(ctx context.Context) error {
        ...
        {{- end }}
        {{- end }}
        return a.{{.name}}DomainService.DeleteMany(ctx, tenantId, vList)
	})
}
...
// @return error
//
func (a *{{.Name}}QueryAppService) FindById(ctx context.Context, aq *appquery.{{.Name}}FindByIdAppQuery) (*view.{{.Name}}View, bool, error) {
	qry := query.New{{.Name}}FindByIdQuery(aq.TenantId, aq.Id)
...
// @return error 错误
//
func (a *{{.Name}}QueryAppService) FindAll(ctx context.Context, aq *appquery.{{.Name}}FindAllAppQuery) ([]*view.{{.Name}}View, bool, error) {
	qry := query.New{{.Name}}FindAllQuery(aq.TenantId)
...
// @return error 错误
//
func (a *{{.Name}}QueryAppService) FindPaging(ctx context.Context, aq *appquery.{{.Name}}FindPagingAppQuery) (*query.{{.Name}}FindPagingResult, bool, error) {
