...
type {{.Name}}Controller struct {
	appService *app_service.{{.Name}}AppQueryService
}

func New{{.Name}}Controller() *{{.Name}}Controller {
	return &{{.Name}}Controller{
		appService: app_service.New{{.Name}}AppQueryService(),
	}
}
...
func (c *{{.Name}}Controller) GetBy{{.AggregateName}}Id(ctx iris.Context, tenantId, {{.aggregateName}}Id string) {
	_, _, _ = restapp.DoQueryList(ctx, func(ctx context.Context) (interface{}, bool, error) {
		return c.appService.FindByList(ctx, tenantId, {{.aggregateName}}Id)
	})
}
